defmodule Mix.Tasks.Phx.Gen.LiveTest do
  ...
  defp copy_new_files(%Mix.Phoenix.Context{} = context, binding, paths) do
    ...
    Mix.Phoenix.copy_from(paths, "priv/templates/phx.gen.live_test", binding, files)
  ...
  defp files_to_be_generated(%Mix.Phoenix.Context{schema: schema, context_app: context_app}) do
    ...
    [
      {:eex, "show_test.ex",
       Path.join([test_prefix, web_path, "live", schema.singular, "show_test.exs"])},
      {:eex, "index_test.ex",
       Path.join([test_prefix, web_path, "live", schema.singular, "index_test.exs"])}
    ]
  end
end
