    ...
    |> case do
      ...
      season ->
        ...
        Core.Repo.transaction(fn ->
          ...
          |> Enum.with_index()
          |> Enum.reduce({cards, []}, fn {rarity_and_pack_slot_caps, slot}, {unpacked_cards, slots} ->
            Logger.debug("Slot #{slot}, unpacked #{length(unpacked_cards)}")
            {available_cards, partial_pack_slots} = Enum.reduce(rarity_and_pack_slot_caps, {unpacked_cards, []}, fn {rarity, pack_slot_cap}, {available_cards, partial_pack_slots} ->
              {matching_cards, unmatched_cards} = Enum.split_with(
                available_cards,
                fn %{rarity_id: rarity_id} -> rarity_id == rarity.id end
              )
              {allocated_cards, remaining_cards} = Enum.split(matching_cards, pack_slot_cap)

              Logger.debug("#{rarity.name} #{pack_slot_cap}, available #{length(available_cards)}, partial #{length(partial_pack_slots)}, matching #{length(matching_cards)}, aonmatching #{length(unmatched_cards)}, allocated #{length(allocated_cards)}, remaining #{length(remaining_cards)}")

              {
                Enum.concat(remaining_cards, unmatched_cards),
                Enum.concat(partial_pack_slots, Enum.map(allocated_cards, fn card -> %{card: card} end))
              }
            end)
