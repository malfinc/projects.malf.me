defmodule PhoenixLiveSession do
  ...
  def get(_conn, sid, opts) do
    Logger.debug("Fetching table name from opts #{sid} #{inspect(opts)}")
    ...
    case :ets.lookup(table, sid) do
      [{^sid, data, _expires_at}] ->
        Logger.debug("PhoenixLiveSession ETS record found")
        :ets.update_element(table, sid, {3, expires_at(opts)})
        {sid, put_meta(data, sid, opts)}

      [] ->
        Logger.debug("PhoenixLiveSession ETS record NOT found")
        {nil, %{}}
    end
  end
  ...
  def maybe_subscribe(socket, %{"__sid__" => sid, "__opts__" => opts})
      when is_binary(sid) and is_list(opts) do
    if LiveView.connected?(socket) do
      PubSub.subscribe(Keyword.fetch!(opts, :pub_sub), "live_session:#{sid}")
