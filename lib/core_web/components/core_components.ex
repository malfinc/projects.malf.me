defmodule CoreWeb.CoreComponents do
  ...
  def flash(assigns) do
    ...
    >
      <h4 :if={@title} class="alert-heading">
        ...
        <.icon :if={@kind == :error} as="exclamation-circle" class="h-4 w-4" />\ <%= @title %>
      </h4>
  ...
  ## Examples
      ...
      <.button>Send!</.button>
      <.button phx-click="go" usable_icon="email">Send!</.button>
  """
  ...
  def button(assigns) do
    ...
    <button
      ...
      class={[
        ...
        "btn",
        @state == "usable" && "btn-#{@as}",
        @state == "busy" && "btn-outline-#{@as}",
        @state == "successful" && "btn-success",
        @state == "failure" && "btn-danger",
        @state == "rejection" && "btn-outline-danger",
        @class
      ]}
      {@rest}
    >
  ...
  def tag(assigns) do
    ~H"""
    <span {@rest} class="badge bg-secondary">
      <%= render_slot(@inner_block) %>
    </span>
    """
  end
